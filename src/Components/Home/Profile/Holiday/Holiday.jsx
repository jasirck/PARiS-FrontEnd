...
import { setProfileHoliday } from "../../../Toolkit/Slice/apiHomeSlice";
...
function Holiday() {
  ...
  const profile_holiday = useSelector((state) => state.api.profile_holiday);
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      if (!profile_holiday) {
        ...
        axios
          .get("api/booked-holidays/", {
            headers: { Authorization: `Bearer ${token}` },
          })
          .then((response) => {
            dispatch(setProfileHoliday(response.data));
            setBookings(response.data);

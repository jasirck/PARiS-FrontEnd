    ...
    const fetchholidayDetails = async () => {
      try {
        const response = await axios.get(
          `/api/holidays-deteils/${holidayId}/`,
          {
            headers: {
        ...
        <div className="bg-white rounded-xl p-6 shadow-md mb-6">
          {days && days.length > 0 ? (
            ...
            <p className="text-[#023246] text-sm text-center">
              No itinerary available for this holiday.
            </p>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          <div className="bg-[#289441] bg-opacity-10 p-6 rounded-xl">
            <h3 className="text-xl font-bold text-[#287094] mb-4 flex items-center">
              <Check size={24} className="mr-2 text-[#287094]" /> holiday
              Includes
            </h3>
            <ul className="space-y-2 text-[#023246]">
              {(typeof holidayData.holiday_included === "string"
                ? JSON.parse(holidayData.holiday_included)
                : holidayData.holiday_included || []
              ).map((item, index) => (
          ...
          <div className="bg-[#942828] bg-opacity-10 p-6 rounded-xl">
            <h3 className="text-xl font-bold text-red-700 mb-4 flex items-center">
              <X size={24} className="mr-2 text-red-700" /> holiday Excludes
            </h3>
            <ul className="space-y-2 text-[#023246]">
              {(typeof holidayData.holiday_excluded === "string"
                ? JSON.parse(holidayData.holiday_included)
                : holidayData.holiday_included || []
              ).map((item, index) => (
                <li key={index}>✗ {item.trim()}</li>
        ...
        <div className="bg-[#D4D4CE] p-6 rounded-xl text-center">
          ...
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-[#287094]">{holidayData.days} Days</p>
            </div>
            <div>
              <p className="text-[#023246] font-semibold">Base  |  Adult | Child </p>
              <p className="text-[#287094] font-bold">
                ₹{holidayData.base_price }  |  ₹  { holidayData.adult_price }  |  ₹  {holidayData.child_price}
              </p>
            </div>
          </div>
          <p className="text-[#023246] mb-6">
            Valid from {holidayData.start} to {holidayData.end}
          </p>
        </div>
      </div>
      <div>

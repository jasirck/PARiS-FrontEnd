...
const HolidaysDeteils = ({ isOpen, onClose, HolidayId }) => {
  const [packageData, setPackageData] = useState(null);
  const [days, setDays] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  useEffect(() => {
    const fetchHolidaysDetails = async () => {
      try {
        const response = await axios.get(`/api/holidays-deteils/${HolidayId}/`, {
          headers: {
    ...
    if (HolidayId) {
      fetchHolidaysDetails();
    }
  }, [HolidayId]);

  const handleBookingModal = (id) => {
    setIsModalOpen(id);
  };
      ...
      <div className="relative bg-[#F6F6F6] w-[95%] md:w-[80%] max-w-[1200px] rounded-2xl shadow-2xl p-6 md:p-8 overflow-y-auto max-h-[90vh]">
        ...
        <div className="bg-[#D4D4CE] p-6 rounded-xl text-center">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div>
              <p className="text-[#023246] font-semibold">
                Base | Adult | Child
              </p>
              <p className="text-[#287094] font-bold">
                ₹{packageData.base_price} | ₹{packageData.adult_price} | ₹
          ...
          <div className="flex justify-center space-x-4">
            <button
              className="px-8 py-3 bg-[#287094] text-white rounded-lg hover:bg-opacity-90 transition duration-300"
              onClick={() => handleBookingModal(packageData.id)}
            >
              Book Now
            </button>
...
export default HolidaysDeteils;
